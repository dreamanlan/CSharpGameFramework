    ...
    public sealed class SkillInstance
    {
        ...
        public List<VisualSkillPropertyInfo> CollectProperties()
        {
            ...
            m_AccessorHelper.VisitProperties((string group, string key, IProperty property) => {
                list.Add(new VisualSkillPropertyInfo { Group = "SkillInstance", Key = key, Accessor = property });
            });
            for (int i = 0; i < m_Sections.Count; ++i) {
                List<VisualSkillPropertyInfo> temp = new List<VisualSkillPropertyInfo>();
                temp.Add(new VisualSkillPropertyInfo { Group = "SkillSection", Key = "Section" + i, Accessor = null });
                m_Sections[i].VisitProperties((string group, string key, IProperty property) => {
                    temp.Add(new VisualSkillPropertyInfo { Group = string.IsNullOrEmpty(group) ? "SkillSection" : group, Key = key, Accessor = property });
                });
                if (temp.Count > 1) {
                    list.AddRange(temp);
                }
            ...
            for (int i = 0; i < m_MessageHandlers.Count; ++i) {
                List<VisualSkillPropertyInfo> temp = new List<VisualSkillPropertyInfo>();
                temp.Add(new VisualSkillPropertyInfo { Group = "SkillMessageHandler", Key = m_MessageHandlers[i].MsgId, Accessor = null });
                m_MessageHandlers[i].VisitProperties((string group, string key, IProperty property) => {
                    temp.Add(new VisualSkillPropertyInfo { Group = string.IsNullOrEmpty(group) ? "SkillMessageHandler" : group, Key = key, Accessor = property });
                });
                if (temp.Count > 1) {
                    list.AddRange(temp);
                }
            ...
            if (null != m_InterruptSection) {
                List<VisualSkillPropertyInfo> temp = new List<VisualSkillPropertyInfo>();
                temp.Add(new VisualSkillPropertyInfo { Group = "SkillMessageHandler", Key = "oninterrupt", Accessor = null });
            ...
            if (null != m_StopSection) {
                List<VisualSkillPropertyInfo> temp = new List<VisualSkillPropertyInfo>();
                temp.Add(new VisualSkillPropertyInfo { Group = "SkillMessageHandler", Key = "onstop", Accessor = null });
