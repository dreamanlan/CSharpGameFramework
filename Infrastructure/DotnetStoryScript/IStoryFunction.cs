...
{
    /// <summary>
    /// Describes the values used in story commands. This interface is used to support parameters, local variables,
    /// global variables and built-in functions (returning the values used in a story command).
    /// </summary>
    ...
    public interface IStoryFunction<T>
    {
        ...
        void Evaluate(StoryInstance instance, StoryMessageHandler handler, BoxedValue iterator, BoxedValueList args);//Parameters are replaced with parameter
                                                                                                                     //values and the value of StoryFunction is calculated
    ...
    public class StoryFunction : IStoryFunction
    {
        public void InitFromDsl(Dsl.ISyntaxComponent param)
        {
        ...
        private StoryFunction NewValueObject()
        {
            StoryFunction obj = new StoryFunction();
            return obj;
        }
        private void CopyFrom(StoryFunction other)
        {
    ...
    public class StoryFunction<T> : IStoryFunction<T>
    {
        public void InitFromDsl(Dsl.ISyntaxComponent param)
        {
            if (IsDslSyntax) {
        ...
        private StoryFunction<T> NewValueObject()
        {
            StoryFunction<T> obj = new StoryFunction<T>();
            return obj;
        }
        private void CopyFrom(StoryFunction<T> other)
        {
            if (null != other.m_Proxy) {
                m_Proxy = other.m_Proxy.Clone();
            }
        ...
        private void CalcInitValue(Dsl.ISyntaxComponent param)
        {
            if (IsDslSyntax) {
                ...
                if (null != val) {
                    ...
                    if (idType == Dsl.ValueData.NUM_TOKEN) {
                        if (id.StartsWith("0x"))
                            SetValue(StoryFunctionHelper.CastTo<T>(uint.Parse(id.Substring(2), System.Globalization.NumberStyles.HexNumber)));
                        else if (id.IndexOf('.') >= 0 || id.IndexOf('e') > 0 || id.IndexOf('E') > 0)
                            SetValue(StoryFunctionHelper.CastTo<T>(float.Parse(id, System.Globalization.NumberStyles.Float)));
                        else
                            SetValue(StoryFunctionHelper.CastTo<T>(int.Parse(id, System.Globalization.NumberStyles.Integer)));
                    }
                    else if (idType == Dsl.ValueData.ID_TOKEN && (id == "true" || id == "false")) {
                        SetValue(StoryFunctionHelper.CastTo<T>(id == "true"));
                    }
                    else {
                        SetValue(StoryFunctionHelper.CastTo<T>(id));
                    }
                }
                else {
#if DEBUG
