    ...
    public class MyClientTashDispatcher
    {
        ...
        private void InitTaskThreads(int threadNum, bool isPassive, int tickSleepTime, int actionNumPerTick)
        {
            ...
            for (int i = 0; i < threadNum; ++i) {
                ...
                if (isPassive) {
                    thread = new MyClientThread(tickSleepTime, actionNumPerTick, m_ActionQueue);//Threads take the initiative to take strategies
                } else {
                    thread = new MyClientThread(tickSleepTime, actionNumPerTick);//Dispatcher actively promotes strategies
                }
                m_Threads[i] = thread;
                thread.Start();
            }
