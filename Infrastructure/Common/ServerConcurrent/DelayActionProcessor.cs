...
{
    public sealed class ServerDelayActionProcessor : IActionQueue
    {
        ...
        {
            bool needUseLambda = true;
            ObjectPool<PoolAllocatedFunc> pool;
            m_ActionPools.GetOrNewData(out pool);
            if (null != pool) {
                PoolAllocatedFunc helper = pool.Alloc();
                if (null != helper) {
                    helper.SetPoolRecycleLock(m_LockForAsyncActionRun);
                    helper.Init(action);
                    m_Actions.Enqueue(helper.Run);
        ...
        public void HandleActions(int maxCount)
        {
            try {
                    ...
                    if (m_Actions.Count > 0) {
                        ...
                        if (null != action) {
                            try {
                                ...
                                LogSystem.Error("ServerDelayActionProcessor action() throw exception:{0}\n{1}", ex.Message, ex.StackTrace);
                            }
                        }
                    } else {
                        break;
                ...
                LogSystem.Error("ServerDelayActionProcessor.HandleActions throw exception:{0}\n{1}", ex.Message, ex.StackTrace);
            }
        }
        ...
        public ServerDelayActionProcessor()
        {
        }

        internal ServerDelayActionProcessor(object lockObj)
        {
