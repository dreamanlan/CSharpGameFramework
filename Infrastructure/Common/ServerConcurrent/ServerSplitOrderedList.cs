...
{
	internal class ServerSplitOrderedList<TKey, T>
	{
		class Node
		{
			public bool Marked;
		...
		ServerSimpleRwLock slim = new ServerSimpleRwLock ();

		readonly IEqualityComparer<TKey> comparer;

		public ServerSplitOrderedList (IEqualityComparer<TKey> comparer)
		{
		...
		public T InsertOrUpdate (uint key, TKey subKey, MyFunc<T> addGetter, MyFunc<T, T> updateGetter)
		{
			Node current;
		...
		public T InsertOrGet (uint key, TKey subKey, T data, MyFunc<T> dataCreator)
		{
			Node current;
		...
		bool InsertInternal (uint key, TKey subKey, T data, MyFunc<T> dataCreator, out Node current)
		{
		...
		public bool CompareExchange (uint key, TKey subKey, T data, MyFunc<T, bool> check)
		{
			Node node;
			uint b = key % (uint)size;
			Node bucket;
		...
		bool ListInsert (Node newNode, Node startPoint, out Node current, MyFunc<T> dataCreator)
		{
