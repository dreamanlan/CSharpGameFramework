...
class(AiGetSkills) {
	static_methods {
		__new_object = deffunc(1)args(...){
			local(__cs2dsl_newobj);__cs2dsl_newobj = newobject(AiGetSkills, "g_AiGetSkills", typeargs(), typekinds(), "ctor", 0, null, ...);
			return(__cs2dsl_newobj);
		}options[needfuncinfo(false)];
		cctor = deffunc(0)args(){
			callstatic(AiGetSkills, "__cctor");
		};
		__cctor = deffunc(0)args(){
			if(AiGetSkills.__cctor_called){
				return();
			}else{
				AiGetSkills.__cctor_called = true;
			};
		}options[needfuncinfo(false)];
	};
	static_fields {
	...
	instance_methods {
		SetProxy = deffunc(0)args(this, result){
			setinstance(SymbolKind.Field, this, AiGetSkills, "m_Proxy", result);
		...
		Clone = deffunc(1)args(this){
			...
			__method_ret_17_4_20_5 = newobject(AiGetSkills, "g_AiGetSkills", typeargs(), typekinds(), "ctor", 0, null);
		...
		Evaluate = deffunc(0)args(this, instance, handler, _params){
			...
			if( execbinary("!=", null, npc, System.Object, System.Object, TypeKind.Class, TypeKind.Class), 26_8_28_9 ){
				getexterninstancestructmember(SymbolKind.Property, getinstance(SymbolKind.Field, this, AiGetSkills, "m_Proxy"), StorySystem.StoryValueResult, "Value") = callexternstaticreturnstruct(BoxedValue, "FromObject", dsltoobject(SymbolKind.Method, true, "BoxedValue:FromObject", callexterninstance(callexterninstance(npc, GameFramework.EntityInfo, "GetSkillStateInfo"), GameFramework.SkillStateInfo, "GetAllSkill")));
			};
		...
		ctor = deffunc(0)args(this){
			callinstance(this, AiGetSkills, "__ctor");
		};
		__ctor = deffunc(0)args(this){
			if(getinstance(SymbolKind.Field, this, AiGetSkills, "__ctor_called")){
				return();
			}else{
				setinstance(SymbolKind.Field, this, AiGetSkills, "__ctor_called", true);
			};
		}options[needfuncinfo(false)];
	};
	instance_fields {
