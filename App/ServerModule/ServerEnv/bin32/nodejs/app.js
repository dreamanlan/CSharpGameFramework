...
var center_client = require('./Release/center_client.node');
center_client.core_start();
...
var VOICE_IP = center_client.core_query_config("voiceip");
var PORT = parseInt(center_client.core_query_config("nodejsport"));
var ENABLE_CLIENT_LOG = 0;
var SHOP_MASK = 0xffffffff;
try {
  ENABLE_CLIENT_LOG = parseInt(center_client.core_query_config("enableclientlog"));
...
try {
  SHOP_MASK = parseInt(center_client.core_query_config("shopmask"));
  if (isNaN(SHOP_MASK)) {
          ...
          if (socket.logicLifeCount <= 0) {
            ...
            if (socketKey) {
              ...
              if (guid) {
                ...
                center_client.core_send_message_by_name("UserSvr", nodeMessageForWrite.toBase64());
...
mainLogic.Start = function () {
  tick_handle = setInterval(function () {
    try {
      if (registered === false) {
        var name = center_client.core_service_name();
        nodeMessageForWrite.initForNodeRegister(MsgEnum.NodeRegister, name);
        var str = nodeMessageForWrite.toBase64();
        center_client.core_send_message_by_name("UserSvr", str);
        console.log("!!post to userserver msg: " + str);
      } else {
        var name = center_client.core_service_name();
        center_client.core_send_command_by_name(name, "Echo");
...
mainLogic.Process = function(handle, session, msg) {
  try {
...
setInterval(() => {
	center_client.core_process_message(mainLogic, mainLogic.Process);
}, 100);
