...
internal static class DBConn
{
    ...
    {
        if (m_MySqlConn == null) {
            LogSys.Log(ServerLogType.INFO, "MySql Connection :{0}", DataCacheConfig.MySqlConnectString);
            try {
                m_MySqlConn = new MySqlConnection(DataCacheConfig.MySqlConnectString);
                m_MySqlConn.Open();
            } catch (System.Exception ex) {
                m_MySqlConn = null;
                LogSys.Log(ServerLogType.ERROR, "MySql Connection ERROR :{0}", ex);
            }
        } else {
            try {
                if (m_MySqlConn != null && m_MySqlConn.State == System.Data.ConnectionState.Closed) {
                    m_MySqlConn.Open();
                    LogSys.Log(ServerLogType.INFO, "MySql connection open again...", DataCacheConfig.MySqlConnectString);
                }
            } catch (System.Exception ex) {
                m_MySqlConn = null;
                LogSys.Log(ServerLogType.ERROR, "MySql Connection ERROR :{0}", ex);
