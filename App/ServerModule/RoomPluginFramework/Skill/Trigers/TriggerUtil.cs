...
namespace GameFramework.Skill.Trigers
{
    public class TriggerUtil
    {
        ...
        public static void Lookat(Scene scene, EntityInfo obj, Vector3 target, float rotateDegree)
        {
        ...
        public static void Lookat(Scene scene, EntityInfo obj, Vector3 target)
        {
            if (scene.EntityController.GetEntityType(obj) == (int)EntityTypeEnum.Tower)
                return;
            Vector3 srcPos = obj.GetMovementStateInfo().GetPosition3D();
        ...
        public static List<EntityInfo> FindTargetInSector(Scene scene, Vector3 center,
                                                      float radius,
                                                      Vector3 direction,
                                                      ...
                                                      float degree)
        {
            ...
            scene.KdTree.QueryWithAction(center.X, center.Y, center.Z, radius, (float distSqr, KdTreeObject kdTreeObj) => {
        ...
        public static void SetObjVisible(EntityInfo obj, bool isShow)
        {
        }

        public static void MoveObjTo(EntityInfo obj, Vector3 position)
        {
            obj.GetMovementStateInfo().SetPosition(position);
        }

        public static float GetObjFaceDir(EntityInfo obj)
        {
        ...
        public static float GetHeightWithGround(EntityInfo obj)
        {
        ...
        public static void AoeQuery(Scene scene, EntityInfo srcObj, EntityInfo targetObj, int aoeType, float range, float angleOrLength, SkillInstance instance, int senderId, int targetType, Vector3 relativeCenter, bool relativeToTarget, MyFunc<float, int, bool> callback)
        {
            ...
            if (aoeType == (int)SkillAoeType.Circle || aoeType == (int)SkillAoeType.Sector) {
                ...
                scene.KdTree.QueryWithFunc(center.X, center.Y, center.Z, range, (float distSqr, KdTreeObject kdTreeObj) => {
                ...
                scene.KdTree.QueryWithFunc(c.X, 0, c.Y, range + angleOrLength / 2, (float distSqr, GameFramework.KdTreeObject kdTreeObj) => {
                    int targetId = kdTreeObj.Object.GetId();
                    if (targetType == (int)SkillTargetType.Enemy && CharacterRelation.RELATION_ENEMY == scene.EntityController.GetRelation(senderId, targetId) ||
                        targetType == (int)SkillTargetType.Friend && CharacterRelation.RELATION_FRIEND == scene.EntityController.GetRelation(senderId, targetId)) {
                        bool isMatch = false;
