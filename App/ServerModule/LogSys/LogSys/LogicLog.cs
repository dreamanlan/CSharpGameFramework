...
public class LogicLog : Log
{
  public LogicLog(string name, int cachesize)
  {
    m_Name = name;
  ...
  public override void CacheLog(string value)
  {
    lock (m_CacheLock) {
      ...
      string buf = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + separator + value + "(" + Thread.CurrentThread.ManagedThreadId + ")" + "\n";
  ...
  public override void DiskLog(string value)
  {
    lock (m_WriteLock) {
      ...
      if (!Directory.Exists("./log/logic/")) {
        Directory.CreateDirectory("./log/logic/");
      }
      char separator = Convert.ToChar(0x01);
  ...
  public override void FlushLog(byte[] ptr, int size, string module_name)
  {
    try {
      ...
      Console.WriteLine("LogicLog::FlushLog exception:{0}\n{1}", ex.Message, ex.StackTrace);
    }
  }
}
