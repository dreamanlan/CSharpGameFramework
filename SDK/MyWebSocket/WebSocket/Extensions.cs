...
namespace WebSocket4Net
{
  public static partial class Extensions
  {
    ...
    public static bool ParseMimeHeader(this string source, WebSocket session, IDictionary<string, object> valueContainer, out string verbLine)
    {
      ...
      while (!string.IsNullOrEmpty(line = reader.ReadLine())) {
        ...
        if (!items.TryGetValue(key, out currentValue)) {
          ...
          if (0 == key.CompareTo("Sec-WebSocket-Accept")) {
            items[key] = value;

            session.FireLog(string.Format("ParseMimeHeader found exist key: Sec-WebSocket-Accept <-> {0}", value));
          } else {
    ...
    internal static bool IsSimpleType(this Type type)
    {
      return
          type.IsValueType ||
          type.IsPrimitive ||
          Array.IndexOf(m_SimpleTypes, type) >= 0 ||
