...
namespace WebSocket4Net.Protocol
{
  ...
  class DraftHybi10Processor : ProtocolProcessorBase
  {
    ...
    private const string c_Error_InvalidHandshake = "invalid handshake";
    ...
    public override bool VerifyHandshake(WebSocket websocket, WebSocketCommandInfo handshakeInfo, out string description)
    {
      ...
      websocket.FireLog(string.Format("VerifyHandshake handshake:{0}, items count:{1}", handshake, websocket.Items.Count));

      if (string.IsNullOrEmpty(handshake)) {
        description = c_Error_InvalidHandshake;
        return false;
      }

      var verbLine = string.Empty;

      if (!handshakeInfo.Text.ParseMimeHeader(websocket, websocket.Items, out verbLine)) {
        description = c_Error_InvalidHandshake;
        return false;
      }

      if (Array.IndexOf(ExptectedResponseVerbLines, verbLine) < 0) {
      ...
      if (!string.IsNullOrEmpty(websocket.SubProtocol)) {
        ...
        if (!websocket.SubProtocol.Equals(protocol, StringComparison.OrdinalIgnoreCase)) {
          description = string.Format("subprotocol doesn't match, support {0} receive {1}", websocket.SubProtocol, protocol);
          return false;
        }
      }
      ...
      if (!expectedAcceptKey.Equals(acceptKey, StringComparison.OrdinalIgnoreCase)) {
        description = string.Format("accept key doesn't match, expected {0} receive {1}", expectedAcceptKey, acceptKey);
        return false;
      }
