...
namespace WebSocket4Net.Protocol
{
  ...
  class DraftHybi00Processor : ProtocolProcessorBase
  {
    ...
    public override bool VerifyHandshake(WebSocket websocket, WebSocketCommandInfo handshakeInfo, out string description)
    {
      ...
      if (challenge.Length != m_ExpectedChallenge.Length) {
        description = string.Format("challenge length doesn't match, expected {0} receive {1}", m_ExpectedChallenge.Length, challenge.Length);
        return false;
      }

      for (var i = 0; i < m_ExpectedChallenge.Length; i++) {
        if (challenge[i] != m_ExpectedChallenge[i]) {
          description = string.Format("challenge doesn't match, {0} expected {1} receive {2}", i, m_ExpectedChallenge[i], challenge[i]);
          return false;
        }
      }
