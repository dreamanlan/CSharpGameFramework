...
namespace Lidgren.Network
{
    ...
    internal bool m_needNotifyLogMessage;
    internal bool m_logMessageReceive;
    internal bool m_logMessageResend;

    public bool LogMessageReceive
    {
      get { return m_logMessageReceive; }
      set { m_logMessageReceive = value; }
    }
    public bool LogMessageResend
    {
      get { return m_logMessageResend; }
      set { m_logMessageResend = value; }
    }
		/// <summary>
      ...
      float delay = 0.025f + (avgRtt * 2.1f); // 25 ms + double rtt
      m_peer.LogWarning("resend delay " + delay + " " + RemoteEndPoint.Address.ToString() + ":" + RemoteEndPoint.Port);
      return delay;
		}
...
#if DEBUG
      m_needNotifyLogMessage = true;
      m_logMessageReceive = true;
      m_logMessageResend = true;
#else
      m_needNotifyLogMessage = false;
      m_logMessageReceive = false;
      m_logMessageResend = false;
#endif
		}
        ...
        return;
			...
			if ((frameCounter % m_infrequentEventsSkipFrames) == 0)
			{
				if (now > m_timeoutDeadline)
				{
					...
					//
          m_peer.LogWarning("Connection timed out at " + now + " deadline was " + m_timeoutDeadline + " " + RemoteEndPoint.Address.ToString() + ":" + RemoteEndPoint.Port);
            ...
            if (m_logMessageReceive) {
              m_peer.LogWarning("Sending ack for " + tuple.Item1 + " #" + tuple.Item2 + " " + RemoteEndPoint.Address.ToString() + ":" + RemoteEndPoint.Port);
            }
            ...
            if (m_logMessageReceive) {
              m_peer.LogWarning("m_queuedOutgoingAcks.Count " + m_queuedOutgoingAcks.Count + " m_sendBufferWritePtr " + m_sendBufferWritePtr + " m_sendBufferNumMessages " + m_sendBufferNumMessages + " " + RemoteEndPoint.Address.ToString() + ":" + RemoteEndPoint.Port);
            }
          ...
          if (chan == null) {
            m_peer.LogWarning("Ignore ack for " + incAck.Item1 + "#" + incAck.Item2 + " " + RemoteEndPoint.Address.ToString() + ":" + RemoteEndPoint.Port);
            continue;
          }
      ...
      if (m_sendBufferWritePtr > 0) {
        if (m_logMessageResend) {
          m_peer.LogWarning("m_sendBufferWritePtr(" + m_sendBufferWritePtr + ") > 0 m_sendBufferNumMessages " + m_sendBufferNumMessages + " " + RemoteEndPoint.Address.ToString() + ":" + RemoteEndPoint.Port);
        }
      ...
      if (sz > m_currentMTU) {
        m_peer.LogWarning("Message " + sz + " larger than MTU " + m_currentMTU + "! Fragmentation must have failed! " + RemoteEndPoint.Address.ToString() + ":" + RemoteEndPoint.Port);
      }

      bool connReset; // TODO: handle connection reset

      if (m_sendBufferWritePtr + sz > m_currentMTU) {
        m_peer.LogWarning("m_sendBufferWritePtr(" + m_sendBufferWritePtr + ") + sz(" + sz + ") > m_currentMTU(" + m_currentMTU + ") m_sendBufferNumMessages " + m_sendBufferNumMessages + " " + RemoteEndPoint.Address.ToString() + ":" + RemoteEndPoint.Port);
        ...
        m_peer.LogWarning("m_sendBufferWritePtr(" + m_sendBufferWritePtr + ") > m_currentMTU(" + m_currentMTU + ") m_sendBufferNumMessages " + m_sendBufferNumMessages + " " + RemoteEndPoint.Address.ToString() + ":" + RemoteEndPoint.Port);
		...
		internal void ReceivedLibraryMessage(NetMessageType tp, int ptr, int payloadLength)
		{
			...
			switch (tp)
			{
				...
				case NetMessageType.LibraryError:
					m_peer.LogError("LibraryError received by ReceivedLibraryMessage; this usually indicates a malformed message");
					break;
				...
				case NetMessageType.NatIntroduction:
					...
					break;
        case NetMessageType.LogMessageReceive:
          m_logMessageResend = true;
          m_logMessageReceive = true;
          m_peer.LogWarning("receive LogMessageReceive from " + RemoteEndPoint.Address.ToString() + ":" + RemoteEndPoint.Port);
          break;
				default:
