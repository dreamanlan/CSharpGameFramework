...
namespace Lidgren.Network
{
  public partial class NetPeer
  {
    ...
    private void BindSocket()
    {
        ...
        try {
          if (m_socket != null) {
            try {
              ...
              LogWarning("Socket.Shutdown exception: " + ex.ToString());
            }

            try {
              m_socket.Close(2);
            } catch (Exception ex) {
              LogWarning("Socket.Close exception: " + ex.ToString());
            }
          }
        } finally {
          m_socket = null;
      ...
      try {
        m_socket.ReceiveBufferSize = m_configuration.SocketReceiveBufferSize;
        m_socket.SendBufferSize = m_configuration.SocketSendBufferSize;
      } catch (Exception ex) {
        LogWarning(string.Format("BindSocket ReceiveBufferSize/SendBufferSize Exception:{0}\n{1}", ex.Message, ex.StackTrace));
      }
      try {
        m_socket.Blocking = false;
      } catch (Exception ex) {
        LogWarning(string.Format("BindSocket Blocking Exception:{0}\n{1}", ex.Message, ex.StackTrace));
      }
      ...
      try {
        ...
        LogWarning(string.Format("BindSocket Bind Exception:{0}\n{1}", ex.Message, ex.StackTrace));
      }
    ...
    private void Heartbeat()
    {
      ...
      if (delta > 0.5) {
        LogWarning("NetPeer.Heartbeat consume " + delta + "s !");
      }
      ...
      if (delta > (1.0 / (double)maxCHBpS) || delta < 0.0) // max connection heartbeats/second max
        ...
        lock (m_connections) {
          int ct = m_connections.Count;
          for (int i = ct - 1; i >= 0; --i) {
            NetConnection conn = m_connections[i];
            conn.Heartbeat(now, m_frameCounter);
            if (conn.m_needNotifyLogMessage) {
              conn.m_needNotifyLogMessage = false;
              
              NetOutgoingMessage om = CreateMessage();
              om.m_messageType = NetMessageType.LogMessageReceive;
              SendLibrary(om, conn.RemoteEndPoint);

              LogWarning("send LogMessageReceive to " + conn.RemoteEndPoint.Address.ToString() + ":" + conn.RemoteEndPoint.Port);
            }
      ...
      do {
        ...
        while ((bytesReceived - ptr) >= NetConstants.HeaderByteSize) {
          ...
          byte payloadLow = m_receiveBuffer[ptr++];
          byte payloadHigh = m_receiveBuffer[ptr++];

          ushort payloadBitLength = (ushort)(payloadLow | (((int)payloadHigh) << 8));
          int payloadByteLength = NetUtility.BytesToHoldBits(payloadBitLength);

          if (bytesReceived - ptr < payloadByteLength) {
            LogWarning("Malformed packet; stated payload length " + payloadByteLength + ", remaining bytes " + (bytesReceived - ptr) + " type " + tp + " seqNum " + sequenceNumber);
            return;
          }
