...
echo building CSharpGameFramework.sln ...
%xbuild% /nologo /noconsolelogger /property:Configuration=%cfg% ^
         /flp:LogFile=%logdir%\CSharpGameFramework.sln.log;Encoding=UTF-8 ^
		     /t:clean;rebuild ^
         %workdir%\CSharpGameFramework.sln
if NOT %ERRORLEVEL% EQU 0 (
  echo build failed, check %logdir%\CSharpGameFramework.sln.log.
  goto error_end
) else (
  echo done.
)
