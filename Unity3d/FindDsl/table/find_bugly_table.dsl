...
{	
    ...
    int("skiprows", 0);
    string("fields", "user,product_version,crash_id,elapsed_time,crash_time,issue_id,is_root,ram,rom,is_new_issue");
...
filter
{
	...
	if(isnullorempty(fields)){
		...
		if($f_stack=="stack" && $f_exception=="exception_message"){
		    var(0) = rowtoline(row, 0, var(2))+","+$device_id+","+$hardware+","+$uos+","+$cpu_name+","+$exception_type+",scene,hz,native,graphics,unknown,pss,vss,mono,"+$f_stack+","+$f_exception;
    		...
    		var(0) = rowtoline(row, 0, var(2))+","+$device_id+","+$hardware+","+$uos+","+$cpu_name+","+$exception_type+","+var(5)+","+var(6)+","+var(7)+","+$f_stack+","+$f_exception;
    	}else{
    	    var(0) = rowtoline(row, 0, var(2))+","+$device_id+","+$hardware+","+$uos+","+$cpu_name+","+$exception_type+",,,,,,,,,"+$f_stack+","+$f_exception;
    	};
