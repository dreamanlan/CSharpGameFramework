...
internal static class TerrainEditUtility
{
    ...
    internal static void Process(GameObject root, Dsl.DslFile file, Dictionary<string, Color32[,]> samplers, Dictionary<string, Size> cacheInfos, List<ObjectInfo> objects)
    {
        if (null != file) {
            ...
            calc.Register("getalpha", "getalpha(ix) api", new ExpressionFactoryHelper<GetAlphaExp>());
            calc.Register("setalpha", "setalpha(ix,val) api", new ExpressionFactoryHelper<SetAlphaExp>());
            calc.Register("clearalpha", "clearalpha() api", new ExpressionFactoryHelper<ClearAlphaExp>());
            calc.Register("getdetail", "getdetail(x,y,ix) api", new ExpressionFactoryHelper<GetDetailExp>());
            calc.Register("samplered", "samplered(key,x,y) api", new ExpressionFactoryHelper<SampleRedExp>());
            calc.Register("samplegreen", "samplegreen(key,x,y) api", new ExpressionFactoryHelper<SampleGreenExp>());
            calc.Register("sampleblue", "sampleblue(key,x,y) api", new ExpressionFactoryHelper<SampleBlueExp>());
            calc.Register("samplealpha", "samplealpha(key,x,y) api", new ExpressionFactoryHelper<SampleAlphaExp>());
            calc.Register("getcache", "getcache(key,x,y) api", new ExpressionFactoryHelper<GetCacheExp>());
            calc.Register("setcache", "setcache(key,x,y,val) api", new ExpressionFactoryHelper<SetCacheExp>());
            calc.Register("addtree", "addtree(ix,x,y,z,f_rot,w_scale,h_scale,u_color,u_lightmap) api", new ExpressionFactoryHelper<AddTreeExp>());
            calc.Register("addobject", "addobject(x,y,z,prefab_str) api", new ExpressionFactoryHelper<AddObjectExp>());
    ...
    private static void ProcessWithDsl(Dsl.FunctionData funcData, string type, float[,] datas, float[, ,] alphamaps, Dictionary<int, int[,]> details, DslCalculator calc, string proc, ref bool resetTrees)
    {
        if (null == funcData)
            return;
        if (null != funcData) {
    ...
    private static void ProcessHeights(float[,] datas, DslCalculator calc, string proc, int x, int y, int w, int h)
    {
    ...
    private static void ProcessHeights(float[,] datas, DslCalculator calc, string proc, int cx, int cy, int r)
    {
        int x = cx - r;
        int y = cy - r;
        int w = r * 2;
    ...
    private static void ProcessAlphamaps(float[,,] alphamaps, DslCalculator calc, string proc, int x, int y, int w, int h)
    {
        int alphanum = alphamaps.GetLength(2);
        for (int ix = 0; ix < w; ++ix) {
            for (int iy = 0; iy < h; ++iy) {
    ...
    private static void ProcessAlphamaps(float[,,] alphamaps, DslCalculator calc, string proc, int cx, int cy, int r)
    {
        int alphanum = alphamaps.GetLength(2);
        int x = cx - r;
        int y = cy - r;
    ...
    private static void ProcessDetails(Dictionary<int, int[,]> details, DslCalculator calc, string proc, int x, int y, int w, int h)
    {
        for (int ix = 0; ix < w; ++ix) {
            for (int iy = 0; iy < h; ++iy) {
                int xi = x + ix;
    ...
    private static void ProcessDetails(Dictionary<int, int[,]> details, DslCalculator calc, string proc, int cx, int cy, int r)
    {
        int x = cx - r;
        int y = cy - r;
        int w = r * 2;
...
internal class GetHeightExp : SimpleExpressionBase
{
...
internal class GetAlphamapExp : SimpleExpressionBase
{
    protected override BoxedValue OnCalc(IList<BoxedValue> operands)
    {
        float r = 0;
...
internal class GetAlphaExp : SimpleExpressionBase
{
    protected override BoxedValue OnCalc(IList<BoxedValue> operands)
    {
        float r = 0;
...
internal class SetAlphaExp : SimpleExpressionBase
{
    protected override BoxedValue OnCalc(IList<BoxedValue> operands)
    {
        double r = 0;
...
internal class ClearAlphaExp : SimpleExpressionBase
{
    protected override BoxedValue OnCalc(IList<BoxedValue> operands)
    {
        int r = 0;
...
internal class GetDetailExp : SimpleExpressionBase
{
    protected override BoxedValue OnCalc(IList<BoxedValue> operands)
    {
        int r = 0;
...
internal class SampleRedExp : SimpleExpressionBase
{
    protected override BoxedValue OnCalc(IList<BoxedValue> operands)
    {
        int r = 0;
...
internal class SampleGreenExp : SimpleExpressionBase
{
    protected override BoxedValue OnCalc(IList<BoxedValue> operands)
    {
        int r = 0;
...
internal class SampleBlueExp : SimpleExpressionBase
{
    protected override BoxedValue OnCalc(IList<BoxedValue> operands)
    {
        int r = 0;
...
internal class SampleAlphaExp : SimpleExpressionBase
{
    protected override BoxedValue OnCalc(IList<BoxedValue> operands)
    {
        int r = 0;
...
internal class GetCacheExp : SimpleExpressionBase
{
    protected override BoxedValue OnCalc(IList<BoxedValue> operands)
    {
        int r = 0;
...
internal class SetCacheExp : SimpleExpressionBase
{
    protected override BoxedValue OnCalc(IList<BoxedValue> operands)
    {
        long r = 0;
...
internal class AddTreeExp : SimpleExpressionBase
{
    protected override BoxedValue OnCalc(IList<BoxedValue> operands)
    {
        long r = 0;
...
internal class AddObjectExp : SimpleExpressionBase
{
    protected override BoxedValue OnCalc(IList<BoxedValue> operands)
    {
