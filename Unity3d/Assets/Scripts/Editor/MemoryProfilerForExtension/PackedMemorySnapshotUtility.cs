...
public static class PackedMemorySnapshotUtility
{
    ...
    static void SaveCrawlerDataToFile(string filePath, PackedCrawlerData packedCrawled)
    {
        ...
        string fileExtension = Path.GetExtension(filePath);
        if (string.Equals(fileExtension, ".datasnap", System.StringComparison.OrdinalIgnoreCase)) {
            System.Runtime.Serialization.Formatters.Binary.BinaryFormatter bf = new System.Runtime.Serialization.Formatters.Binary.BinaryFormatter();
            using (Stream stream = File.Open(filePath, FileMode.Create)) {
            ...
            using (TextWriter writer = File.CreateText(filePath)) {
                var errors = new List<string>();
                var serializer = getSerializer(errors);
                serializer.Serialize(writer, packedCrawled);
                logErrors(errors);
            }
        }
    ...
    internal static PackedCrawlerData LoadCrawlerDataFromFile()
    {
        var filePath = EditorUtility.OpenFilePanelWithFilters("Load CrawlerData", previousDirectory, new[] { "CrawlerDatas", "datasnap3,datasnap2,datasnap" });
        if (string.IsNullOrEmpty(filePath))
            return null;

        previousDirectory = Path.GetDirectoryName(filePath);
        Debug.LogFormat("Loading \"{0}\"", filePath);
        var packedSnapshot = LoadCrawlerDataFromFile(filePath);
        Debug.LogFormat("Completed loading \"{0}\"", filePath);
        return packedSnapshot;
    }
    ...
    static PackedCrawlerData LoadCrawlerDataFromFile(string filePath)
    {
        Debug.LogFormat("Loading...");
        System.Diagnostics.Stopwatch stopwatch = new System.Diagnostics.Stopwatch();
        PackedCrawlerData result;
        string fileExtension = Path.GetExtension(filePath);

        if (string.Equals(fileExtension, ".datasnap3", System.StringComparison.OrdinalIgnoreCase)) {
            Profiler.BeginSample("PackedMemorySnapshotUtility.LoadFromFile(litjson)");
            stopwatch.Start();
            ...
            using (TextReader reader = File.OpenText(filePath)) {
                var errors = new List<string>();
                var serializer = getSerializer(errors);
                result = (PackedCrawlerData)serializer.Deserialize(reader, typeof(PackedCrawlerData));
                logErrors(errors);
            }
            ...
            var json = File.ReadAllText(filePath);
            result = JsonUtility.FromJson<PackedCrawlerData>(json);
            ...
            var binaryFormatter = new System.Runtime.Serialization.Formatters.Binary.BinaryFormatter();
            using (Stream stream = File.Open(filePath, FileMode.Open)) {
                result = binaryFormatter.Deserialize(stream) as PackedCrawlerData;
            }
