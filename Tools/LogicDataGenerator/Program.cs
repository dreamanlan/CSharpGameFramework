...
namespace LogicDataGenerator
{
        ...
        internal bool Init(string dslFile)
        {
            ...
            try {
                ...
                foreach (var dslInfo in file.DslInfos) {
                    ...
                    if (null == info) {
                        LogSystem.Error("{0} must end with ';' , line {1} file {2}", dslInfo.GetId(), dslInfo.GetLine(), dslFile);
                        haveError = true;
                        continue;
                    }
                    if (info.GetId() == "explanationfile") {
                        FunctionData call = info;
                                    ...
                                    if (null != item) {
                                        if (item.HaveExternScript()) {
                                            if (item.GetId() == "message2logic") {
                                                typeConverter.m_Message2LogicCode = item.GetParamId(0);
                                            }
                                            else if (item.GetId() == "logic2message") {
                                                typeConverter.m_Logic2MessageCode = item.GetParamId(0);
                                            }
                                            else if (item.GetId() == "crudcode") {
                                                typeConverter.m_IsSpecial = true;
                                                typeConverter.m_CrudCode = item.GetParamId(0);
                                            }
                                        }
                                        else {
                                            FunctionData converterData = item;
                                            if (converterData.GetParamNum() == 1) {
            ...
            if (funcData.IsHighOrder) {
                ...
                if (null != callData && callData.GetParamNum() >= 1) {
                    ...
                    foreach (ISyntaxComponent comp in funcData.Params) {
                        FunctionData field = comp as FunctionData;
                        if (null != field) {
                            if (field.IsHighOrder) {
                                if (field.GetId() == "enum") {
                                    ParseEnum(dslFile, field, messageDef.m_GroupName, messageDef.m_EnumTypes, ref haveError);
                                }
                                else if (field.GetId() == "message") {
                                    ParseMessage(dslFile, field, messageDef.m_GroupName, true, messageDef.m_Messages, ref haveError);
                                }
                                else if (field.GetId() == "member") {
                                    FunctionData customField = field.LowerOrderFunction;
                                    if (null != customField && customField.GetParamNum() >= 3) {
                                        ...
                                        foreach (ISyntaxComponent comp2 in field.Params) {
    ...
    internal class Program
    {
        ...
        private static void GenServerData()
        {
            ...
            parser.GenAllMessageWraps("DataProto/DataWrap.cs", string.Empty);
